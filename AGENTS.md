...
- **Project Board:** Ticket status is tracked in the public GitHub Project [Metio Development](https://github.com/users/nbyl/projects/7) via its `Status` field (`Backlog`, `Ready`, `In progress`, `In review`, `Done`). Whenever these instructions talk about moving a ticket "in progress" or "in review", this means setting the issue's `Status` on this project (e.g. add the issue with `gh project item-add 7 --owner nbyl --url <issue-url>`, then update the Status field).
...
- **Pull Requests:** Create a pull request on GitHub and set the issue's status in the "Metio Development" project to "In review" if applicable.
