...
variable "controller_image" {
  ...
  type        = string
  default     = "europe-docker.pkg.dev/metio-distribution/metio/controller:2.1.0" # x-release-please-version
}

variable "machine_agent_image" {
  description = "Docker image for metio-machine-agent. Override to pin a specific version."
  type        = string
  default     = "europe-docker.pkg.dev/metio-distribution/metio/machine-agent:2.1.0" # x-release-please-version
}

variable "backup_image" {
  description = "Docker image used for the per-server backup service (mc-backup)."
  type        = string
  default     = "europe-docker.pkg.dev/metio-distribution/metio/mc-backup:2.1.0" # x-release-please-version
}
...
variable "daprd_image" {
  ...
  type        = string
  default     = "europe-docker.pkg.dev/metio-distribution/metio/daprd:2.1.0" # x-release-please-version
}
...
variable "backup_deleted_server_retention_days" {
  description = "How long backups are kept after a server is deleted (ADR-0004). Passed to the controller as BACKUP_DELETED_SERVER_RETENTION_DAYS."
  type        = number
  default     = 30
...
variable "backup_cleanup_schedule" {
  description = "Cloud Scheduler cron expression driving the controller's backup cleanup sweep (POST /api/backups/cleanup)."
  type        = string
  default     = "0 * * * *"
}
